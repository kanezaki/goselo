...
kernel void localresponsenormalization_e9042b681e4c6b45bc3a5dcd3c0b551853c64b8a1e1a213adc48ed96(device float * static_buffer[[buffer(0)]],
                          device float * dynamic_buffer[[buffer(1)]],
                          const device int * meta_buffer [[buffer(2)]],
                          uint index[[thread_position_in_grid]],
                          uint num_threads[[threads_per_grid]])
{
    ...
    const int Phalfn = meta_buffer[5];
    const float Pk = *((const device float *)(& meta_buffer[6]));
    const float Palpha = *((const device float *)(& meta_buffer[7]));
    const float Pmbeta = *((const device float *)(& meta_buffer[8]));
